...
/**
 * 	\file       htdocs/core/modules/product/doc/pdf_answer.modules.php
 * 	\ingroup    societe
 ...
 */
class pdf_answer extends ModelePDFProduct
{
	...
	public function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "Answer";
		$this->description = $langs->trans("DocumentModelAnswer");
...
//						}
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $titlekey = "")
	{
		...
		$pdf->SetXY($posx, $posy);
		$pdf->SetTextColor(0, 0, 60);
		$title = $outputlangs->transnoentities($titlekey).' '.$questionnaire->ref;
		...
		$pdf->SetXY($posx, $posy);
		$pdf->SetTextColor(0, 0, 60);
		$pdf->MultiCell(100, 4, $outputlangs->transnoentities("Title")." : ".$outputlangs->convToOutputCharset($questionnaire->title), '', 'R');
		...
		if (!empty($object->fk_element))
		{

			if ($object->type_element == 'contact')
			{
				dol_include_once('/contact/class/contact.class.php');
				$dest = new Contact($db);
				$dest->fetch($object->fk_element);
				$name = $dest->lastname.' '.$dest->firstname;
			}
			else if ($object->type_element == 'thirdparty')
			{
			...
			else if ($object->type_element == 'user')
			{
			...
			if (!empty($dest->socname))
			{
				$posy += 5;
				$pdf->SetXY($posx, $posy);
				$pdf->SetTextColor(0, 0, 60);
				$pdf->MultiCell(100, 3, $outputlangs->transnoentities("Company")." : ".$outputlangs->convToOutputCharset($dest->socname), '', 'R');
			}
			if (!empty($name))
			{
				$posy += 4;
				$pdf->SetXY($posx, $posy);
				$pdf->SetTextColor(0, 0, 60);
				$pdf->MultiCell(100, 3, $outputlangs->transnoentities("Name")." : ".$outputlangs->convToOutputCharset($name), '', 'R');
			}
			if (!empty($object->email))
			{
				...
				$pdf->SetXY($posx, $posy);
				$pdf->SetTextColor(0, 0, 60);
				$pdf->MultiCell(100, 3, $outputlangs->transnoentities("Email")." : ".$outputlangs->convToOutputCharset($object->email), '', 'R');
			}
		}
